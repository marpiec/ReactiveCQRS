...
abstract class FirstCommand[AGGREGATE_ROOT, RESPONSE <: CommandResponse] {
  val userId: UserId
}

abstract class ConcurrentCommand[AGGREGATE_ROOT, RESPONSE <: CommandResponse] {
  val userId: UserId
  val aggregateId: AggregateId
}

abstract class Command[AGGREGATE_ROOT, RESPONSE <: CommandResponse] {
  val userId: UserId
  val aggregateId: AggregateId
 ...
 */
trait CommandLogTransform[AGGREGATE_ROOT, RESPONSE <: CommandResponse] { self: Command[AGGREGATE_ROOT, RESPONSE] =>
 ...
 * password from command before storing it.
 */
trait FirstCommandLogTransform[AGGREGATE_ROOT, RESPONSE <: CommandResponse] { self: FirstCommand[AGGREGATE_ROOT, RESPONSE] =>
