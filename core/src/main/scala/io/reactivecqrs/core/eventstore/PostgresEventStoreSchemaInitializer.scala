...
class PostgresEventStoreSchemaInitializer  {
  ...
  private def dropExistingFunctions(): Unit = DB.autoCommit { implicit session =>
    sql"""DROP FUNCTION IF EXISTS add_event(BIGINT, BIGINT, BIGINT, INT, INT, VARCHAR(128), TIMESTAMP, VARCHAR(10240))""".execute().apply()
    sql"""DROP FUNCTION IF EXISTS add_undo_event(BIGINT, BIGINT, BIGINT, INT, INT, VARCHAR(128), TIMESTAMP, VARCHAR(10240), INT)""".execute().apply()
    sql"""DROP FUNCTION IF EXISTS add_duplication_event(BIGINT, BIGINT, BIGINT, INT, INT, VARCHAR(128), TIMESTAMP, VARCHAR(10240), BIGINT, INT)""".execute().apply()
  }

  private def createAddEventFunction(): Unit = DB.autoCommit { implicit session =>
    SQL("""
          |CREATE OR REPLACE FUNCTION add_event(command_id BIGINT, user_id BIGINT, aggregate_id BIGINT, expected_version INT, aggregate_type_id INT, event_type VARCHAR(128), event_time TIMESTAMP, event VARCHAR(10240))
          |RETURNS BIGINT AS
          |$$
          |DECLARE
          |    current_version INT;
  ...
  private def createAddUndoEventFunction(): Unit = DB.autoCommit { implicit session =>
    SQL("""
          |CREATE OR REPLACE FUNCTION add_undo_event(command_id BIGINT, user_id BIGINT, _aggregate_id BIGINT, expected_version INT, aggregate_type_id INT, event_type VARCHAR(128), event_time TIMESTAMP, event VARCHAR(10240), undo_count INT)
          |RETURNS BIGINT AS
          |$$
          |DECLARE
          |    current_version INT;
  ...
  private def createAddDuplicationEventFunction(): Unit = DB.autoCommit { implicit session =>
    SQL("""
          |CREATE OR REPLACE FUNCTION add_duplication_event(command_id BIGINT, user_id BIGINT, aggregate_id BIGINT, expected_version INT, aggregate_type_id INT, event_type VARCHAR(128), event_time TIMESTAMP, event VARCHAR(10240), _base_id BIGINT, _base_version INT)
          |RETURNS BIGINT AS
          |$$
          |DECLARE
          |    current_version INT;
