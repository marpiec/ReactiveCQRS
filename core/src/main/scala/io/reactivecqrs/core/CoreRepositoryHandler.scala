...
class CoreRepositoryHandler[AGGREGATE](aggregateRepositoryActor: ActorRef) extends RepositoryFirstEventHandler[AGGREGATE] with RepositoryFollowingEventHandler[AGGREGATE]{

  implicit val timeout = Timeout(1 second)

  override def storeFirstEvent(commandId: CommandId, userId: UserId, newAggregateId: AggregateId, event: Event[AGGREGATE]): Result[Unit, CqrsException] = {
  ...
  override def storeFollowingEvent(commandId: CommandId, userId: UserId, aggregateId: AggregateId, expectedVersion: AggregateVersion, event: Event[AGGREGATE]): Result[Unit, CqrsException] = {
